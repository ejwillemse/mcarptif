...
class NetworkExtract:
    ...
    def update_cost_matrix_circular(self):
        ...
        self.d[self.depot_arc_index, :] = self.d_np_req_orig[self.depot_arc_index, :].copy() / self.travel_speed
        self.d[:, self.depot_arc_index] = self.d_np_req_orig[:, self.depot_arc_index].copy() / self.travel_speed
        self.d[self.if_arc_index, :] = self.d_np_req_orig[self.if_arc_index, :].copy() / self.travel_speed
        self.d[:, self.if_arc_index] = self.d_np_req_orig[:, self.if_arc_index].copy() / self.travel_speed
        if self.round_cost:
            self.d = self.d.astype(int)
    ...
    def deconstruct_solution(self, node_list, arc_list):
        ...
        def sp_path_to_arcs(sp_path):
            ...
            arc_frame['u-v'] = arc_frame['u'].astype(int).astype(str) + '-' + \
        ...
        travel_paths = travel_paths.merge(arc_list[['geometry',
                                                    'length',
                                                    'u-v',
                                                    'maxspeed']],
                                          how='left',
        ...
        travel_paths = travel_paths.drop_duplicates(['solution_group',
                                                     'u-v'],
                                                    keep='first')
    ...
    def full_producer_report(self, bc=False, bc_full=False):
        ...
        route_kpi = df_arc_producer[['arc_index', 'Vehicle', 'route', 'subroute', 'Collection day',
             'index_temp', 'time', 'Name', 'Type', 'Address']]
        #df_arc_producer = df_arc_producer.dropna(subset=['category'])
        if not bc:

            df_arc_producer = df_arc_producer[
                ['Vehicle', 'route', 'Collection day', 'Service time window',
                 'Postal Code', 'Name', 'Type', 'Address',
                 'demand', 'total units',
        ...
        if not bc:
            ...
            df_arc_final = df_arc_final[['Vehicle', 'route', 'Collection day', 'Service time window',
                                         'Name', 'Type', 'Address', 'Postal Code']]
        ...
        self.producer_collection_schedule = self.producer_collection_schedule.dropna(subset=['Name'])
    ...
    def store_results(self,
        ...
        r.save_to_html(file_name='scenario_maps_html/{} - Collection maps.html'.format(scenario_name),
                       data=data_out,
                       read_only=True,
                       config=config)
