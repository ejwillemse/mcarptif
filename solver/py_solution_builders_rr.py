...
class build_solutions(object):
    ...
    def insert_IFs_depot(self, route):
        depot = self.depot
        if_arc = self.if_arc
        if route[0][0] == depot: return route
    ...
    def build_CLARPIF_list(self, solution):
        ...
        for i in xrange(nRoutes):
            ...
            routes.append([])
            for j in xrange(solution[i]['nTrips']):
    ...
    def build_CARP_cum_list(self, solution):
        ...
        for i in xrange(nRoutes):
            ...
            for j in xrange(1,len(routes[-1])):
                arc = routes[-1][j]
    ...
    def buil_cumulitive_lists(self, solution_lists):
        ...
        for r in xrange(len(routes)):
            ...
            for trip in range(1, nTrips):
                l = 0
                for j in xrange(ifs[trip - 1] + 1, ifs[trip] + 1):
                    c += self.d[route[j - 1]][route[j]] + service_a[j]
                    cost_c.append(c)
                    l += load_a[j]
                    load_c.append(l)
...
def build_CLARPIF_list(solution):
    ...
    for i in xrange(nRoutes):
        ...
        for j in xrange(len(solution[i]['Trips'])):
