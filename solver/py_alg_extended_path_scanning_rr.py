...
class TieBreakRules(object):
    ...
    def hybrid(self, nearestArcsL, vehicleSubTripLoad):
        ...
        else: return(self.mindepot(nearestArcsL))
    ...
    def hybridIF(self, nearestArcsL, vehicleSubTripLoad):
        ...
        else: return(self.minIF(nearestArcsL))
    ...
    def hybridIFdepot(self, nearestArcsL, vehicleSubTripLoad, vehicleCost):
        ...
        else: return(self.hybridIF(nearestArcsL, vehicleSubTripLoad))
    ...
    def randomrule(self, nearestArcsL, vehicleSubTripLoad):
        ...
        '''
        if vehicleSubTripLoad == None: rulen = np.random.randint(1, 5)
        else: rulen = np.random.randint(1, 6)
        if rulen == 1: return(self.maxdepot(nearestArcsL))
        if rulen == 2: return(self.mindepot(nearestArcsL))
        if rulen == 3: return(self.maxyield(nearestArcsL))
        if rulen == 4: return(self.minyield(nearestArcsL))
        if rulen == 5: return(self.hybrid(nearestArcsL, vehicleSubTripLoad))        
    ...
    def randomruleIFs_sub(self, nearestArcsL, vehicleSubTripLoad, vehicleCost):
        ...
        if rulen == 1: return(self.maxIF(nearestArcsL))
        if rulen == 2: return(self.minIF(nearestArcsL))
        if rulen == 3: return(self.maxyield(nearestArcsL))
        if rulen == 4: return(self.minyield(nearestArcsL))
        if rulen == 5: return(self.hybridIFdepot(nearestArcsL, vehicleSubTripLoad, vehicleCost))   
    ...
    def randomruleAll(self, nearestArcsL, vehicleSubTripLoad, vehicleCost):
        ...
        '''
        if vehicleSubTripLoad == None: rulen = np.random.randint(1, 8)
        else: rulen = np.random.randint(1, 9)
        if rulen == 1: return(self.maxdepot(nearestArcsL))
        if rulen == 2: return(self.mindepot(nearestArcsL))
        if rulen == 3: return(self.maxIF(nearestArcsL))
        if rulen == 4: return(self.minIF(nearestArcsL))
        if rulen == 5: return(self.maxyield(nearestArcsL))
        if rulen == 6: return(self.minyield(nearestArcsL))
        if rulen == 7: return(self.hybrid(nearestArcsL, vehicleSubTripLoad))
        if rulen == 8: return(self.hybridIFdepot(nearestArcsL, vehicleSubTripLoad, vehicleCost)) 
    ...
    def choose_arc(self, nearestArcsL, vehicleSubTripLoad, vehicleCost, rule='MaxDepo'):
        ...
        '''
        if rule == 'MaxDepo': return(self.maxdepot(nearestArcsL))
        if rule == 'MinDepo': return(self.mindepot(nearestArcsL))
        if rule == 'MaxIF': return(self.maxIF(nearestArcsL))
        if rule == 'MinIF': return(self.minIF(nearestArcsL))
        if rule == 'MaxYield': return(self.maxyield(nearestArcsL))
        if rule == 'MinYield': return(self.minyield(nearestArcsL))
        
        if rule == 'Hybrid': return(self.hybrid(nearestArcsL, vehicleSubTripLoad))
        if rule == 'HybridIFdepto': return(self.hybridIFdepot(nearestArcsL, vehicleSubTripLoad, vehicleCost))
        
        if rule == 'RandomRule': return(self.randomrule(nearestArcsL, vehicleSubTripLoad))
        if rule == 'RandomRuleIFs': return(self.randomruleIFs(nearestArcsL, vehicleSubTripLoad, vehicleCost))
        if rule == 'RandomRuleAll': return(self.randomruleAll(nearestArcsL, vehicleSubTripLoad, vehicleCost))
        if rule == 'RandomArc': return(self.randomarc(nearestArcsL))
...
class ExtendedPathScanning_Operators(object):
    ...
    def testtriplimit(self, arc, arcdist):
        ...
        '''
        if (self.routecost + arcdist + self.serveCost[arc] + self.if_cost[arc][self.end_depot]) <= self.maxTrip:
            return(True)
        else: return(False)
    ...
    def testtriplimit_est(self, arc, arcdist, estimate_if_cost):
        ...
        Test if an arc can be added without exceeding max trip length.
        '''
        if (self.routecost + arcdist + self.serveCost[arc] + self.if_cost[arc][self.end_depot] + estimate_if_cost) <= self.maxTrip:
            return(True)
        else: return(False)

    def c_findnearestarcs(self, previousarc):
        (nearestarcs, nearest) = c_alg_extended_path_scanning.findnearestarcs(previousarc,   
    ...
    def c_findnearestarcs_elipse(self, previousarc):
        (nearestarcs, nearest, k) = c_alg_extended_path_scanning.findnearestarcs_elipse(previousarc,   
                                                                              self.reqArcsUnassigned, 
                                                                              self.tripload, 
                                                                              self.routecost,
                                                                              self._tc, self._ned)
    ...
    def findnearestarcs_elipse(self, previousarc):
        ...
        for nextarc in self.reqArcsUnassigned:
            ...
            if (disttoarc + serveC + disttodepot) <= (self._tc/self._ned + disttodepot_direct):
                k += 1
                if disttoarc <= nearest:
                    loadflag = self.testload(nextarc)
                    triplimitflag = self.testtriplimit(nextarc, disttoarc)
    ...
    def c_findnearestarcs_cap_elipse(self, previousarc):
        (nearestarcs, nearest, k) = c_alg_extended_path_scanning.findnearestarcs_cap_elipse(previousarc,   
                                                                              self.reqArcsUnassigned, 
                                                                              self.tripload, 
                                                                              self.routecost,
                                                                              self._tc, self._ned, self.info.IFarcsnewkey)
        return(nearestarcs, nearest, k)

    def c_findnearestarcs_noload(self, previousarc, estimate_if_cost):
        (nearestarcs, nearest) = c_alg_extended_path_scanning.findnearestarcs_noload(previousarc,   
                                                                                      self.reqArcsUnassigned, 
                                                                                      self.tripload, 
                                                                                      self.routecost, 
                                                                                      estimate_if_cost)
    ...
    def findnearestarcs_noload(self, previousarc, estimate_if_cost):
        ...
        for nextarc in self.reqArcsUnassigned:
            ...
            if disttoarc <= nearest:
                triplimitflag = self.testtriplimit_est(nextarc, disttoarc, estimate_if_cost)
                if triplimitflag:
                    if disttoarc < nearest:
                        nearest = disttoarc
                        nearestarcs = [nextarc]
    ...
    def c_findnearestIFarcs(self, previousarc):
        (nearestarcs, nearest) = c_alg_extended_path_scanning.findnearestIFarcs(previousarc,
                                                                                self.reqArcsUnassigned,
                                                                                self.tripload, 
                                                                                self.routecost)
    ...
    def c_findnearestIFarcs_elipse(self, previousarc):
        (nearestarcs, nearest, k) = c_alg_extended_path_scanning.findnearestIFarcs_elipse(previousarc,
                                                                                            self.reqArcsUnassigned,
                                                                                            self.tripload, 
                                                                                            self.routecost,
                                                                                            self._tc,
    ...
    def findnearestIFarcs_elipse(self, previousarc):
        ...
        for nextarc in self.reqArcsUnassigned:
            ...
            disttodepot = self.if_cost[nextarc][self.depot]
            serveC = self.serveCost[nextarc]
            if (disttoarc + serveC + disttodepot) <= 1.5*(self._tc/self._ned + disttodepot_direct):
                k += 1
                if disttoarc <= nearest:
                    triplimitflag = self.testtriplimit(nextarc, disttoarc)    
                    if triplimitflag:
    ...
    def c_findnearestarcs_nolimit(self, previousarc):
        (nearestarcs, nearest) = c_alg_extended_path_scanning.findnearestarcs_nolimit(previousarc, 
                                                                                      self.reqArcsUnassigned,
                                                                                      self.tripload)
        return(nearestarcs, nearest)
    
    def c_findnearestarcs_nolimit_elipse(self, previousarc):
        (nearestarcs, nearest, k) = c_alg_extended_path_scanning.findnearestarcs_nolimit_elipse(previousarc, 
                                                                                                  self.reqArcsUnassigned,
                                                                                                  self.tripload,
                                                                                                  self._tc, 
                                                                                                  self._ned)
    ...
    def c_findnearestarcsbalanced_sub1(self, previousarc):
        (nearestarcs, nearest) = c_alg_extended_path_scanning.findnearestarcs_sub1(previousarc, self.reqArcsUnassigned)
        return(nearestarcs, nearest)
    ...
    def c_checknearestarcs(self, nearestarcs, nearest):
        nextarcfine = c_alg_extended_path_scanning.checknearestarcs(nearestarcs, 
                                                                      nearest, 
                                                                      self.tripload,  
                                                                      self.routecost)
        return(nextarcfine)
    ...
    def checknearestarcs(self, nearestarcs, nearest):
        ...
        for arc in nearestarcs:
            if (self.testload(arc) == True) & (self.testtriplimit(arc, nearest) == True): 
    ...
    def c_checknearestarcs_noload(self, nearestarcs, nearest, estimate_if_cost):
        ...
        Check which nearest arcs does not exceed capacity and maxtrip restrictions.
        '''
        nextarcfine = c_alg_extended_path_scanning.checknearestarcs_noload(nearestarcs, 
                                                                          nearest, 
                                                                          self.tripload, 
                                                                          self.routecost,
                                                                          estimate_if_cost)
    ...
    def checknearestarcs_nolimits(self, nearestarcs):
        ...
        nextarcfine = []
        for arc in nearestarcs:
            if self.testload(arc): nextarcfine.append(arc)
        return(nextarcfine)
    ...
    def findnearestarcsbalanced_noload(self, previousarc, estimate_if_cost):
        ...
        if self.c_modules:
            (nearestarcs, nearest) = self.c_findnearestarcsbalanced_sub1(previousarc)
            nearestarcs = self.c_checknearestarcs_noload(nearestarcs, nearest, estimate_if_cost)
        else:
            (nearestarcs, nearest) = self.findnearestarcsbalanced_sub1(previousarc)
            nearestarcs = self.checknearestarcs_noload(nearestarcs, nearest, estimate_if_cost)
        return(nearestarcs, nearest)
...
class EPS(ExtendedPathScanning_Operators, TieBreakRules):
        ...
        while True:
            ...
            if self.c_modules:
                if self._check_elipse_rule(self.tripload):
                    (nearestarcs, nearest, k) = self.c_findnearestarcs_nolimit_elipse(previousarc)
                else:
                    (nearestarcs, nearest) = self.c_findnearestarcs_nolimit(previousarc)
            else:
                if self._check_elipse_rule(self.tripload):
                    (nearestarcs, nearest, k) = self.findnearestarcs_nolimit_elipse(previousarc)
                else:
            ...
            elif len(nearestarcs) > 1: # Multiple closest arcs exisits, so need to break ties.
                vehicleCost=0
                chosearc = self.choose_arc(nearestarcs, self.tripload, vehicleCost, self.rule)
    ...
    def buildMultipleRoutes(self, balanced = False):
        ...
        while self.reqArcsUnassigned:
            ...
            solution[nVehicles] = {}
            if balanced: self.buildbalancedroute()
            else: self.buildroute()
    ...
    def EPS_five_rules(self, balanced=False):
        ...
        for i, rule in enumerate(self.classic_rules):
            ...
            if not self.reduce_all_trips: solutions.append(deepcopy(s1))
            ...
            incumbent_solutions = evaluatesolutions.update_all_incumbents(s1, incumbent_solutions)
            ...
            if self.reduce_all_trips: 
                ...
                incumbent_solutions_reduce_routes = evaluatesolutions.update_all_incumbents(s2, incumbent_solutions_reduce_routes)
    ...
    def EPS_random(self, rule='RandomArc', nSolutions=1000, outFileName='test_error.csv'):
        '''
    ...
    def tester(self, solution):
        if self.testsolutions: test_solution(self.info, solution)
    ...
    def EPS_solver(self, rule_type, nSolutions=5, outFileName='test_error.csv'):
        ...
        if self._got_to_scan: scan_mean = np.mean(self._got_to_scan)
        else: scan_mean = 0
        if self.elipse_forced: elipse_forced_mean = self.elipse_forced/nSolutions/self._ned
        else: elipse_forced_mean = 0
        return(sol)

    def EPS_break_even_solver(self, rule_type, balanced='Original'):
        if balanced == 'Original': balanced = False
        else: balanced = True
...
class EPS_IF(ExtendedPathScanning_Operators, TieBreakRules):
    ...
    def __init__(self, info):
        ...
        self.rule = 'MaxDepo'
        self.classic_rules = ['MaxDepo', 'MinDepo', 'MaxYield', 'MinYield', 'Hybrid']
        self.classic_if_rules = ['MaxDepo', 'MinDepo', 'MaxYield', 'MinYield', 'HybridIFdepto']
        self.IF_rules = ['MaxIF', 'MinIF', 'MaxYield', 'MinYield', 'HybridIFdepto']
        self.all_rules = ['MaxDepo', 'MinDepo','MaxIF', 'MinIF', 'MaxYield', 'MinYield', 'Hybrid', 'HybridIFdepto']
        self.testsolutions = False
        self.info = info
        self.bestSol = 1e300000
        self.bestSolN = 1e300000
    ...
    def reset_trip(self, trip):
        ...
        self.routeLoadsLists.append(self.tripload); self.tripload = 0 
        self.routeCostsLists.append(self.tripcost); self.tripcost = 0 
        self.routeDeadheadLists.append(self.tripdeadhead); self.tripdeadhead = 0
        self.routeServiceCostLists.append(self.tripservicecosts); self.tripservicecosts = 0
    ...
    def buildIFroute(self):
        ...
        while True:
            ...
            if elipse_cost_flag:
                if self.c_modules:
                    (nearestarcs, nearest, k) = self.c_findnearestarcs_elipse(previousarc)
                else:
...
#             elif elipse_capacity_flag:
            if elipse_capacity_flag:
                if self.c_modules:
                    (nearestarcs, nearest, k) = self.c_findnearestarcs_cap_elipse(previousarc)
                else:
                    (nearestarcs, nearest, k) = self.findnearestarcs_cap_elipse(previousarc)
                #self._got_to_scan.append(k/self._ned_left)
            else:
                if self.c_modules:
            ...
            if len(nearestarcs) == 1: # Arc can be added without exceeding capacity.
                ...
                chosearc = self.choose_arc(nearestarcs, self.tripload, self.routecost, self.rule)
                trip = self.addarc(chosearc, nearest, trip)
            else: # trip is full
#                 if elipse_cost_flag:
#                     if self.c_modules:
...
#                 else:
                ...
                if nearestarcs: # arc can be added after IF visit without exceeding max route length.
                    self.ntrips += 1
                    if len(nearestarcs) == 1: 
                        chosearc = nearestarcs[0]
                    elif len(nearestarcs) > 1: # Multiple closest arcs exists, so need to break ties.
                        chosearc = self.choose_arc(nearestarcs, self.tripload, self.routecost, self.rule)
                    trip = self.addIFarc(chosearc, nearest, trip) # Add IF visit to end of trip
        ...
        while True:
            ...
            if len(nearestarcs) == 1: # Arc can be added without exceeding capacity.
                ...
                if nearestarcs: # arc can be added after IF visit without exceeding max route length.
                    ...
                    if len(nearestarcs) == 1: 
                        chosearc = nearestarcs[0]
                    elif len(nearestarcs) > 1: # Multipl closest arcs exists, so need to break ties.
                        chosearc = self.choose_arc(nearestarcs, self.tripload, self.routecost, self.rule)
                    trip = self.addIFarc(chosearc, nearest, trip) # Add IF visit to end of trip
                    self.reset_trip(trip)
                    trip = [trip[-1]] # begin new trip with IF visit
    ...
    def buildMultipleIFRoutes(self, outFileName = None, balanced = False):
        ...
        while self.reqArcsUnassigned:
            ...
            #print('\t Vehicle Route - %i'%nVehicles)
            solution[nVehicles] = {}
            if balanced: self.buildbalancedIFroute()
            else: self.buildIFroute()
    ...
    def EPS_IF_rules(self, rules, balanced=False):
        ...
        for rule in rules:
            ...
            t = clock()
            s = self.buildMultipleIFRoutes(balanced)
            if self.improve_IFs: s = improve_IFs.improve_solution(s)
        ...
        if self.reduce_best_trips: self.bestVehComp = self.Reduce_Trips.reduce_trip(self.bestVehComp)[0]
       
        if self.improve_IFs:part.free_c_local_search(True)
    ...
    def EPS_IF_solver(self, rule_type, nSolutions=5, outFileName = None, balanced = 'Original'):
        if balanced == 'Original': balanced = False
        else: balanced = True
...
class EPS_GiantRoute(ExtendedPathScanning_Operators, TieBreakRules):
    ...
    def buildroute(self, rule = 'MaxDepo'):
        ...
        while self.reqArcsUnassigned:
            previousarc = trip[-1]
            if self.c_modules: (nearestarcs, nearest) = self.c_findnearestarcsbalanced_sub1(previousarc)
            else: (nearestarcs, nearest) = self.findnearestarcsbalanced_sub1(previousarc)
            if len(nearestarcs) == 1: # Arc can be added without exceeding capacity.
                trip = self.addarc(nearestarcs[0], nearest, trip)
            elif len(nearestarcs) > 1: # Multiple closest arcs exisits, so need to break ties.
...
class EPS_MultipleGiantRoute(ExtendedPathScanning_Operators, TieBreakRules):
    ...
    def estimate_if_cost(self, trip):
        self.stats = py_solution_stats.route_IF_cost_stats(trip[1:], self.d, self.if_cost)
    ...
    def buildIFroute(self):
        ...
        while True:
            previousarc = trip[-1]
            if len(trip) > 2: average_if_cost = self.estimate_if_cost(trip)
            else: average_if_cost = 0
            if self.c_modules:
                (nearestarcs, nearest) = self.c_findnearestarcs_noload(previousarc, average_if_cost)
            else:
                (nearestarcs, nearest) = self.findnearestarcs_noload(previousarc, average_if_cost)
            if len(nearestarcs) == 1: # Arc can be added without exceeding capacity.
                trip = self.addarc(nearestarcs[0], nearest, trip)
            elif len(nearestarcs) > 1: # Multiple closest arcs exisits, so need to break ties.
                chosearc = self.choose_arc(nearestarcs, self.tripload, self.routecost, self.rule)
    ...
    def buildbalancedIFroute(self):
        ...
        while True:
            ...
            (nearestarcs, nearest) = self.findnearestarcsbalanced_noload(previousarc, average_if_cost)
            if len(nearestarcs) == 1: # Arc can be added without exceeding capacity.
                trip = self.addarc(nearestarcs[0], nearest, trip)
            elif len(nearestarcs) > 1: # Multiple closest arcs exisits, so need to break ties.
                chosearc = self.choose_arc(nearestarcs, self.tripload, self.routecost, self.rule)
    ...
    def partition_routes(self, solution):
        ...
        for i in xrange(nVehicles):
            complete_route.append(self.partition.gen_solution_list(solution[i]['Trips'][0][1:-2])[0])
...
def test_merge(problem_set = 'Centurion'):
    problems = ['mval_IF_3L', 'Lpr_IF', 'Lpr_IF_075L']#[ 'gdb_IF', 'bccm_IF', 'gdb_IF_3L', 'bccm_IF_3L','mval_IF_15L', 'mval_IF_3L', 'Lpr_IF', 'Lpr_IF_075L']
    for problem_set in problems:
        problem_list = py_return_problem_data.return_problem_data_list(problem_set)
        problem_list.sort()
        extension   = ['_Balanced_v2','_Normal_v2']#['_Normal_NR','_Balanced_NR', '_Balanced_v2','_Normal_v2']
        extension2  = ['_RR', '_RA']
        for filename in problem_list:
            for ext in extension:
                for ext2 in extension2:
                    #solve_problem(problem_set, filename)
                    name = 'Prelim_test_output/' + problem_set + '/' + filename[:-len('_problem_info.dat')] + ext2 + ext + '.csv'
                    ...
                    if (ext == '_Balanced_NR')|(ext == '_Normal_NR'): NNS.reduce_all_trips = False
                    if (ext == '_Balanced_v2')|(ext == '_Normal_v2'): NNS.reduce_all_trips = True
                    if (ext == '_Balanced_NR')|(ext == '_Balanced_v2'): balanced = True
                    if (ext == '_Normal_NR')|(ext == '_Normal_v2'): balanced = False
                    if ext2 == '_RR': NNS.EPS_IF_solver(rule_type = 'EPS_IF_random_five_rules', nSolutions = 200, balanced = balanced)
                    if ext2 == '_RA': NNS.EPS_IF_solver(rule_type = 'EPS_random_arc', nSolutions = 200, balanced = balanced)
